...
function filteredIdeas() {
  ...
  return ideas.filter((idea) => {
    const categoryMatch =
      state.category === "all" ||
      (state.category === "saved" ? state.saved.has(idea.id) : idea.category === state.category);
  ...
  return `
    <article class="idea-card" style="${categoryStyle(idea.category)}">
      ...
      <div class="card-details" id="details-${escapeHtml(idea.id)}" hidden>
        ...
        ${caution}
        <div class="source-row"><span class="source-pill">From: ${escapeHtml(idea.source)}</span></div>
      </div>
    </article>
  `;
}
